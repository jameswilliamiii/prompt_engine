namespace :active_prompt do
  ...
  task eval_demo: :environment do
    ...
    begin
      ...
      puts "\n--- View in Browser ---"
      puts "Eval Set: http://localhost:3000/active_prompt/prompts/#{prompt.id}/eval_sets/#{eval_set.id}"
      puts "Eval Run: http://localhost:3000/active_prompt/prompts/#{prompt.id}/eval_runs/#{eval_run.id}"
