...
module ActivePrompt
  RSpec.describe "EvalRuns", type: :request do
    ...
    describe 'GET /active_prompt/prompts/:prompt_id/eval_runs/:id' do
      context 'with a pending eval run' do
        ...
        it 'returns a success response' do
          get active_prompt.prompt_eval_run_path(prompt, pending_run)
      ...
      context 'with a running eval run' do
        ...
        it 'returns a success response' do
          get active_prompt.prompt_eval_run_path(prompt, running_run)
          expect(response).to be_successful
        end
      ...
      context 'with a completed eval run' do
        ...
        it 'returns a success response' do
          get active_prompt.prompt_eval_run_path(prompt, completed_run)
          expect(response).to be_successful
        end
      ...
      context 'with a failed eval run' do
        ...
        it 'returns a success response' do
          get active_prompt.prompt_eval_run_path(prompt, failed_run)
          expect(response).to be_successful
        end
      ...
      context 'with OpenAI integration data' do
        ...
        it 'displays link to OpenAI report when available' do
          get active_prompt.prompt_eval_run_path(prompt, openai_run)
      ...
      it 'displays the prompt version number' do
        get active_prompt.prompt_eval_run_path(prompt, eval_run)
        expect(response.body).to include("v#{prompt_version.version_number}")
      end
      
      it 'includes a back link to the eval set' do
        get active_prompt.prompt_eval_run_path(prompt, eval_run)
        expect(response.body).to include(active_prompt.prompt_eval_set_path(prompt, eval_set))
      end
    end
  end
end