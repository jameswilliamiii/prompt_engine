...
module PromptEngine
  RSpec.describe "EvalSets", type: :request do
    ...
    describe "GET /prompt_engine/prompts/:prompt_id/eval_sets" do
      ...
      it "returns a success response" do
        get prompt_engine.prompt_eval_sets_path(prompt)
    ...
    describe "GET /prompt_engine/prompts/:prompt_id/eval_sets/:id" do
      let!(:test_case1) { create(:test_case, eval_set: eval_set) }
      let!(:test_case2) { create(:test_case, eval_set: eval_set) }

      it "returns a success response" do
        get prompt_engine.prompt_eval_set_path(prompt, eval_set)
        expect(response).to be_successful
      end
    ...
    describe "GET /prompt_engine/prompts/:prompt_id/eval_sets/new" do
      it "returns a success response" do
        get prompt_engine.new_prompt_eval_set_path(prompt)
        expect(response).to be_successful
      end
    end
    ...
    describe "GET /prompt_engine/prompts/:prompt_id/eval_sets/:id/edit" do
      it "returns a success response" do
        get prompt_engine.edit_prompt_eval_set_path(prompt, eval_set)
        expect(response).to be_successful
      end
    end
    ...
    describe "DELETE /prompt_engine/prompts/:prompt_id/eval_sets/:id" do
      let!(:eval_set_to_delete) { create(:eval_set, prompt: prompt) }

      it "destroys the eval_set" do
        expect {
          delete prompt_engine.prompt_eval_set_path(prompt, eval_set_to_delete)
        }.to change(EvalSet, :count).by(-1)
      end

      it "redirects to the eval_sets list" do
        delete prompt_engine.prompt_eval_set_path(prompt, eval_set_to_delete)
        expect(response).to redirect_to(prompt_engine.prompt_eval_sets_path(prompt))
      end
    end

    describe "POST /prompt_engine/prompts/:prompt_id/eval_sets/:id/run" do
      ...
      before do
        ...
        allow_any_instance_of(PromptEngine::EvaluationRunner).to receive(:execute).and_return(true)
      end

      it "creates a new eval run" do
        expect {
          post prompt_engine.run_prompt_eval_set_path(prompt, eval_set)
      ...
      it "calls EvaluationRunner" do
        expect_any_instance_of(PromptEngine::EvaluationRunner).to receive(:execute)
        post prompt_engine.run_prompt_eval_set_path(prompt, eval_set)
      end
      ...
      it "redirects to the eval run" do
        post prompt_engine.run_prompt_eval_set_path(prompt, eval_set)
        eval_run = EvalRun.last
        expect(response).to redirect_to(prompt_engine.prompt_eval_run_path(prompt, eval_run))
      end

      context "when evaluation fails" do
        before do
          allow_any_instance_of(PromptEngine::EvaluationRunner).to receive(:execute).and_raise(StandardError, "API Error")
        end
